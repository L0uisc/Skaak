...
void geeFoutboodskap(Fout fout, bool geldigRoep=false, int pos=0)
{
    ...
    switch (fout)
    {
        ...
        case Fout::TE_VEEL_KOORDINATE:
            foutboodskap = "te veel ko�rdinate in die skuif.\n"
                "'n Skuif kan 'n maksimum van twee ko�rdinate bevat: een oorsprong en een bestemming.";
            break;
...
bool isGeldig(bool isWit, const std::string &skuif)
{
    ...
    if (geldig)
    {
        ...
        for (int k { 0u }; k < skuifLengte; ++k)
        {
            ...
            case Karakter::GELID:  // Voer uit as huidige karakter (skuif[k]) op 'n ko�rdinaat volg.
            ...
            case Karakter::PROMOVEER:  // Voer uit as huidige karakter (skuif[k]) op 'n '=' volg.
                // 'n Pion kan net promoveer op die laaste gelid. As die ko�rdinaat 'n ander gelid gee,
                // l� die fout reeds by daardie karakter in die skuifstring.
                  ...
                  int afstand=0)
{
    ...
    // Sorg dat die ko�rdinate nie af van die bord af gaan nie.
    while (soekPos.gelid < g_sylengte && soekPos.ry < g_sylengte)
    {
        // As daar 'n stuk is op die posisie, return die ko�rdinaat.
